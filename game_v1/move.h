...
void move_with_keyboard(int n,int player, int& pos1_x, int& pos1_y, int& pos_x, int& pos_y, int recentlymove[][5],int player1_color,int player2_color,int state[][25],int camp[][25],int leave_camp_value,int turn,int is_comp,char name[]) {
	char ch;
	int playerR;
	if (player == 1)
		playerR = 2;
	...
	do
	{
		...
		if (Q_key(ch)) {
			if (check_draw(recentlymove))
				showStatus(n,playerR,3,player1_color,player2_color,name,is_comp);
